 ...
 * @LastEditors: zfd
 * @LastEditTime: 2020-12-01 16:29:37
 * @Description:街道审核列表
-->
...
<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" class="design-table" :data="list" element-loading-text="Loading" fit highlight-current-row :default-sort="{prop: 'statusId', order: 'ascending'}" @row-dblclick="flowView">
      ...
      <el-table-column label="申请时间" min-width="145" prop="addTime" sortable align="center">
        <template slot-scope="scope">
          <i class="el-icon-time" />
          <span>{{ new Date(scope.row.addTime) | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="最新处理时间" min-width="145" prop="updateTime" sortable align="center">
        <template v-if="scope.row.updateTime" slot-scope="scope">
          <i class="el-icon-time" />
          <span>{{ new Date(scope.row.updateTime) | parseTime('{y}-{m}-{d} {h}:{i}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="状态" prop="statusId" sortable min-width="110" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" label="操作" min-width="200">
        <template slot-scope="scope">
          <el-row type="flex" justify="space-around">
            <el-button v-if="scope.row.statusId === 8" size="mini" type="warning" @click="$router.push({name:'StreetCheck',params:{applyId:scope.row.Id,statusId:scope.row.statusId}})">
              审 核
            </el-button>
            <el-button v-if="scope.row.statusId === 11" size="mini" type="warning" @click="$router.push({path:'/street/record_fault',query:{applyId:scope.row.Id}})">
              违规记录
            </el-button>
            <el-button v-if="scope.row.statusId === 11" size="mini" type="warning" @click="$router.push({path:'/street/handle_fault',query:{applyId:scope.row.Id}})">
              查看回复
            </el-button>
          </el-row>
        </template>
